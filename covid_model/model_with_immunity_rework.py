...
class CovidModelWithVariants(CovidModel):
    ...
    def build_ode(self):
        ...
        # vaccination
        for i in {1, 2, 3}:
            self.add_flows_by_attr({'vacc': f'shot{i-1}' if i > 0 else 'none'}, {'vacc': f'shot{i}', 'immun': f'imm{i}'}, constant=f'shot{i}_per_available * shot{i}_fail_rate', from_pool=True)
            self.add_flows_by_attr({'vacc': f'shot{i-1}' if i > 0 else 'none'}, {'vacc': f'shot{i}'}, constant=f'shot{i}_per_available * (1 - shot{i}_fail_rate)', from_pool=True)
        # disease lifecycle
        ...
        # immunity decay
        self.add_flows_by_attr({'immun': 'imm3'}, {'immun': 'imm2'}, coef='1 / 200')
        self.add_flows_by_attr({'immun': 'imm2'}, {'immun': 'imm1'}, coef='1 / 200')
        self.add_flows_by_attr({'immun': 'imm1'}, {'immun': 'none'}, coef='1 / 200')
    ...
    def build_SR_to_E_ode(self):
        ...
        for variant in self.attributes['variant'][1:]:
            ...
            self.add_flows_by_attr({'seir': 'S'}, {'seir': 'E', 'variant': variant}, coef='betta * (1 - ef) / total_pop',
    ...
    def y0_dict(self):
        ...
        y0d[('I', '40-64', 'none', 'none', 'none')] = 2.2
