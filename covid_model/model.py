...
class CovidModel:
    ...
    def hosp_reporting_frac_by_t(self):
        ...
        hrf = hrf.ffill()
        hrf.index.name = 'date'
        hrf = pd.concat([hrf.assign(region=r) for r in self.attrs['region']]).set_index('region',
    ...
    def hosp_reporting_frac(self):
        ...
        """
        return self.__hosp_reporting_frac if self.__hosp_reporting_frac is not None else {
            dt.datetime.strftime(self.start_date, "%Y-%m-%d"): 1}
    ...
    def modeled_vs_observed_hosps(self):
        ...
        """
        #hrf_finder
        #change .rename('modeled_actual') to .rename('modeled_observed')
        ...
        for variant in self.attrs['variant']:
            ...
            if self.mobility_mode is None or self.mobility_mode == "none":
                for region in self.attrs['region']:
                    ...
                    self.add_flows_from_attrs_to_attrs({'seir': 'S', 'region': region}, {'seir': 'E', 'variant': variant}, to_coef='lamb * betta', from_coef=f'(1 - immunity) / region_pop', scale_by_attrs={'seir': 'I', 'variant': variant, 'region': region})
                    self.add_flows_from_attrs_to_attrs({'seir': 'S', 'region': region}, {'seir': 'E', 'variant': variant}, to_coef='betta', from_coef=f'(1 - immunity) / region_pop', scale_by_attrs={'seir': 'A', 'variant': variant, 'region': region})
                    # exposure attributable to immune escape (regardless of immunity)
                    self.add_flows_from_attrs_to_attrs({'seir': 'S', 'region': region}, {'seir': 'E', 'variant': variant}, to_coef='lamb * betta', from_coef=f'immunity / region_pop', from_to_coef='immune_escape', scale_by_attrs={'seir': 'I', 'variant': variant, 'region': region})
                    self.add_flows_from_attrs_to_attrs({'seir': 'S', 'region': region}, {'seir': 'E', 'variant': variant}, to_coef='betta', from_coef=f'immunity / region_pop', from_to_coef='immune_escape', scale_by_attrs={'seir': 'A', 'variant': variant, 'region': region})
                ...
                for infecting_region in self.attrs['region']:
                    for susceptible_region in self.attrs['region']:
                        self.add_flows_from_attrs_to_attrs({'seir': 'S', 'region': susceptible_region}, {'seir': 'E', 'variant': variant}, to_coef='lamb * betta', from_coef=f'mob_{susceptible_region}_exposure_from_{infecting_region} * (1 - immunity) / region_pop', scale_by_attrs={'seir': 'I', 'variant': variant, 'region': infecting_region})
                        self.add_flows_from_attrs_to_attrs({'seir': 'S', 'region': susceptible_region}, {'seir': 'E', 'variant': variant}, to_coef='betta', from_coef=f'mob_{susceptible_region}_exposure_from_{infecting_region} * (1 - immunity) / region_pop', scale_by_attrs={'seir': 'A', 'variant': variant, 'region': infecting_region})
                        self.add_flows_from_attrs_to_attrs({'seir': 'S', 'region': susceptible_region}, {'seir': 'E', 'variant': variant}, to_coef="lamb * betta", from_coef=f'mob_{susceptible_region}_exposure_from_{infecting_region} * immunity / region_pop', from_to_coef='immune_escape', scale_by_attrs={'seir': 'I', 'variant': variant, 'region': infecting_region})
                        self.add_flows_from_attrs_to_attrs({'seir': 'S', 'region': susceptible_region}, {'seir': 'E', 'variant': variant}, to_coef="betta", from_coef=f'mob_{susceptible_region}_exposure_from_{infecting_region} * immunity / region_pop', from_to_coef='immune_escape',scale_by_attrs={'seir': 'A', 'variant': variant, 'region': infecting_region})
            # Transmission parameters attached to the transmission location
            elif self.mobility_mode == "location_attached":
                for infecting_region in self.attrs['region']:
                    for susceptible_region in self.attrs['region']:
                        for transmission_region in self.attrs['region']:
                            self.add_flows_from_attrs_to_attrs({'seir': 'S', 'region': susceptible_region}, {'seir': 'E', 'variant': variant}, to_coef='lamb * betta', from_coef=f'mob_{transmission_region}_frac_from_{infecting_region} * mob_{susceptible_region}_frac_in_{transmission_region} * (1 - immunity) / region_pop', scale_by_attrs={'seir': 'I', 'variant': variant, 'region': infecting_region})
                            self.add_flows_from_attrs_to_attrs({'seir': 'S', 'region': susceptible_region}, {'seir': 'E', 'variant': variant}, to_coef='betta', from_coef=f'mob_{transmission_region}_frac_from_{infecting_region} * mob_{susceptible_region}_frac_in_{transmission_region} * (1 - immunity) / region_pop', scale_by_attrs={'seir': 'A', 'variant': variant, 'region': infecting_region})
                            self.add_flows_from_attrs_to_attrs({'seir': 'S', 'region': susceptible_region}, {'seir': 'E', 'variant': variant}, to_coef="lamb * betta", from_coef=f'mob_{transmission_region}_frac_from_{infecting_region} * mob_{susceptible_region}_frac_in_{transmission_region} * immunity / region_pop',from_to_coef='immune_escape', scale_by_attrs={'seir': 'I', 'variant': variant, 'region': infecting_region})
                            self.add_flows_from_attrs_to_attrs({'seir': 'S', 'region': susceptible_region}, {'seir': 'E', 'variant': variant}, to_coef="betta", from_coef=f'mob_{transmission_region}_frac_from_{infecting_region} * mob_{susceptible_region}_frac_in_{transmission_region} * immunity / region_pop', from_to_coef='immune_escape', scale_by_attrs={'seir': 'A', 'variant': variant, 'region': infecting_region})
