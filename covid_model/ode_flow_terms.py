...
class ScaledODEFlowTerm(ODEFlowTerm):
    ...
    def add_to_nonlinear_matrices(self, matrices, t_int):
        if self.coef_by_t[t_int] != 0:
            matrices[tuple(self.scale_by_cmpts_idxs)][self.from_cmpt_idx, self.from_cmpt_idx] -= self.coef_by_t[
                t_int]
...
class WeightedScaledODEFlowTerm(ODEFlowTerm):
    ...
    def add_to_nonlinear_matrices(self, matrices, t_int):
        scale_by_cmpts_coef_by_t = self.scale_by_cmpts_coef_by_t[t_int] if self.scale_by_cmpts_coef_by_t else [
                                                                                                                  1] * len(
            self.scale_by_cmpts_idxs)
        for scale_by_cmpt_idx, scale_by_cmpt_coef in zip(self.scale_by_cmpts_idxs, scale_by_cmpts_coef_by_t):
            matrices[scale_by_cmpt_idx][self.from_cmpt_idx, self.from_cmpt_idx] -= self.coef_by_t[
                                                                                       t_int] * scale_by_cmpt_coef
            matrices[scale_by_cmpt_idx][self.to_cmpt_idx, self.from_cmpt_idx] += self.coef_by_t[
                                                                                     t_int] * scale_by_cmpt_coef
