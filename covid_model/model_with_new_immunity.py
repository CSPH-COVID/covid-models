...
class CovidModelWithVariants(CovidModel):
    ...
    attr = OrderedDict({'seir': ['S', 'E', 'I', 'A', 'Ih', 'D'],
                        ...
                        'vacc': ['unvacc', 'dose1', 'dose2', 'dose3'],
                        'variant': ['none', 'wt', 'omicron'],
                        'immun': ['none', 'partial', 'full', 'full_plus']})
    ...
    #     for variant in self.attributes['variant']:
    #         # seed variants
    #         self.add_flow(('S', '40-64', 'unvacc', 'none', 'none'), ('E', '40-64', 'unvacc', variant, 'none'), constant='om_seed')
    #
    #         # transmission
    #         infectious_cmpts = self.filter_cmpts_by_attrs({'seir': 'I'}) + self.filter_cmpts_by_attrs({'seir': 'I'})
    #         infectious_cmpt_coefs = ['lamb' if seir == 'I' else '1' for seir, age, vacc, variant, immun in infectious_cmpts]
    #         for age in self.attributes['age']:
    #             for vacc in self.attributes['vacc']:
    #                 for recovered_variant in self.attributes['variant']:
    #
    #                     immunity = 'base_immunity ** variant_immune_escape_k'
    #                     if variant == 'omicron' and recovered_variant != 'omicron' and 'immun' == 'partial':
    #                         immunity += '* '
    #                     transm = f'betta * (1 - {immunity}'
    #                     self.add_flow(('S', age, vacc, recovered_variant), ('E', age, vacc, variant), ,
