...
class ModelSpecsArgumentParser(ArgumentParser):
    def __init__(self):
        ...
        self.add_argument('-in', '--increment', type=int, help="if not specifying tslices, need to specify increment")
        ...
        self.add_argument('-rg', '--regions', nargs='+', type=str, help="regions to be run, default is no regions (statewide model)")
        self.add_argument("-rfsid", "--region_fit_spec_ids", type=int, nargs="+", help="spec_id's to pull the fitted TC's for each region from")
        ...
        self.set_defaults(refresh_actual_vacc=False, refresh_actual_mobility=False, mobility_mode='none', start_date=None)
    ...
    def non_specs_args_as_dict(self, update=None):
        update = update if update is not None else {}
        return {**{k: v for k, v in self.parse_known_args()[0].__dict__.items() if k not in self.specs_args}, **update}
