...
def main():
    ...
    df10 = df[df[0] == '10_percent']

    # plot the projection along with the uncertainty in x

    from_date = dt.datetime.strptime("2021-12-01", "%Y-%m-%d")
    to_date = dt.datetime.strptime("2022-07-15", "%Y-%m-%d")
    dfo2 = dfo[(dfo.date >= from_date) & (dfo.date <= to_date)]

    for dfp, sens in zip([df5, df10], ["5", "10"]):
        dfp2 = dfp[(dfp.date >= from_date) & (dfp.date <= to_date)]
        ...
        for j, high_low, col in zip(['0', '1'], ['High', 'Low'], ['darkorange', 'blue']):
            dfp2_sens = dfp2[(dfp2[1] == j) & ~(dfp2[[2, 3]] == ['0', '0']).all(axis=1)]
            dfp2_med = dfp2[(dfp2[[1, 2, 3]] == [j, '0', '0']).all(axis=1)]
            for series in dfp2_sens.series.unique():
                ax.plot(dfp2[dfp2.series == series].date, dfp2[dfp2.series == series].hospitalized, alpha=0.3, color=col, lw=0.8)
            dfp2_med.to_csv(get_filepath_prefix(outdir) + fname_prefix + f"_hospitalized_{sens}_{high_low}.csv")
            dfp2_sens.to_csv(get_filepath_prefix(outdir) + fname_prefix + f"_hospitalized_{sens}_{high_low}_sensitivity.csv")

            ax.plot(dfp2_med.date, dfp2_med.hospitalized, color=col, label=f"{high_low} BA.2.12.1 Immune Escape", lw=2.0)
